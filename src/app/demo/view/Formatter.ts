            ...
            for(let i = 0; i < data[0].length; i++){
                ...
                if (data[0][i].match(/(?<=#)(.*?)(?=#)/g) !== null){
                    ...
                    switch (codeFormattage){
                        ...
                        case 'JJMM': 
                                    ...
                                    for(let j = 1; j < data.length ; j++){
                                        ...
                                        if(data[j][i] === 19000000){ data[j][i] = ""; }
                                            ...
                                            data[j][i]= data[j][i].toString().slice(2, 4) +"/" +data[j][i].toString().slice(4, 6) 
                        ...
                        case 'KMON' :
                                    for(let j = 1; j < data.length ; j++){
                                        if(!( data[j][i] === undefined || data[j][i].length == 0  )){data[j][i] = (data[j][i]/1000).toFixed(2).toString() + " K" }
                        ...
                        case 'MON' :
                            for(let j = 1; j < data.length ; j++){
                                if(!( data[j][i] === undefined || data[j][i].length == 0  )){data[j][i] = (data[j][i]).toFixed(2).toString()}
                        ...
                        case 'MON+' :
                            for(let j = 1; j < data.length ; j++){
                                if(!( data[j][i] === undefined || data[j][i].length == 0  )){
                                    if(data[j][i] >= 0){
                                        data[j][i] = (data[j][i]).toFixed(2).toString();
                                        colorMapper.set(data[j][i], '#32CD32');
                                    }else{
                                        data[j][i] = (data[j][i]).toFixed(2).toString();
                                        colorMapper.set(data[j][i], '#FF0000');
                                    }
